...
- (void)viewDidLoad {
    ...
    //[self queryButtonTapped];
    /*
    [self chargeSqliteTime:^{
        NSLog(@"创建成功了");
    }];
     */
...
- (void)createdTableView
{
    ...
    expImageView.image = [UIImage imageNamed:shareInfo.expCode];
    ...
    expNameLabel.text = shareInfo.expressName;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ResultTableViewCell *cell = [tableView dequeueReusableHeaderFooterViewWithIdentifier:identityCell];
    if (!cell) {
        cell = [[ResultTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identityCell];
    }
    ...
    //NSLog(@"timerArr:%@",[timeArr objectAtIndex:timeArr.count - 1]);
    
    //[[DateConvertManager dateFormatter]setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    //NSDate  *date = [[DateConvertManager dateFormatter]dateFromString:@"2015-03-09 00:10:17"];
